	...
	public function load($moduleConfig){
		EventManager::listen(Twigger::EVENT_TWIG_ENVIRONMENT_CREATED, function () use ($moduleConfig){
			...
			if (is_array($moduleConfig)){
				if (array_key_exists('twig-sources', $moduleConfig)) foreach ($moduleConfig['twig-sources'] as $namespace => $source) Twigger::Service()->addPath($source, $namespace);
				if (array_key_exists('twig-source', $moduleConfig)) Twigger::Service()->addPath($moduleConfig['twig-source'], '__main__');
			}
		});
	}
